...
class HTML4DefinitionTests(unittest.TestCase):
    ...
    def setUp(self):
        self.html4_colors = {}
        soup = BeautifulSoup(urllib2.urlopen('http://www.w3.org/TR/html401/types.html'))
        color_table = soup.find('table',
                                attrs={'summary': 'Table of color names and their sRGB values'})
...
class CSS21DefinitionTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.css21_colors = {}
        soup = BeautifulSoup(urllib2.urlopen('http://www.w3.org/TR/CSS2/syndata.html'))
        ...
        for color_square in color_table.findAll('span', attrs={'class': 'colorsquare'}):
            color_name, color_value = self.color_matching_re.match(color_square.text).groups()
...
class CSS3DefinitionTests(unittest.TestCase):
    ...
    def setUp(self):
        self.css3_colors = {}
        soup = BeautifulSoup(urllib2.urlopen('http://www.w3.org/TR/css3-color/'))
        ...
        rgb_values = [td.text for \
                      td in color_table.findAll('td',
                                                attrs={'class': 'c',
                                                       'style': 'background:silver'}) \
                      if not td.text.startswith('#') and not td.text.startswith('&')]
        for i, color_name in enumerate(color_names):
            self.css3_colors[color_name] = {'hex': hex_values[i],
