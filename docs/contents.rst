...
:class:`~typing.NamedTuple` instances are also provided to
...
Additionally, to aid in type annotations, the following type aliases
are defined, and used throughout this module:
...
.. warning:: Previously there were two competing HTML5 standards: one
   from WHATWG, and one from W3C. The WHATWG version is now the sole
   official HTML5 standard, and so the functions documented below
   implement the HTML5 color algorithms as given in `section 2.3.6 of
   the WHATWG HTML5 standard
